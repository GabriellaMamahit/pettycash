            ...
            <div class="card">
                ...
                <div class="card-body pt-0">
                    <div class="mb-3">
                        ...
                        $alamat_user = strtolower($this->fungsi->user_login()->address_user);

                        // ✅ Mapping langsung di sini tanpa fungsi luar
                        $jenis_saldo_login = null;
                        switch ($alamat_user) {
                            case 'jakarta':
                                $jenis_saldo_login = 'JKT';
                                break;
                            case 'balikpapan':
                                $jenis_saldo_login = 'BPP';
                                break;
                            case 'karimun':
                                $jenis_saldo_login = 'TBK';
                                break;
                            case 'galang':
                                $jenis_saldo_login = 'LU';
                                break;
                            case 'sekupang':
                                $jenis_saldo_login = 'PA';
                                break; // atau biarkan = null kalau tidak perlu
                        }

                        // ✅ Jika Sekupang: tombol langsung aktif tanpa pengecekan
                        if ($alamat_user !== 'sekupang') {

                            if ($jenispettycashcek === 'BMG') {
                                $isDisabled = true;
                                $popoverContent = "Tidak dapat mengajukan reimbursement - anda tidak memiliki akses";
                            } else {
                                $cek = $this->db->where('jenis_saldo', $jenispettycashcek)
                                ...
                                if ($cek < 1) {
                                    $isDisabled = true;
                                    $popoverContent = "Upload dan submit data BPKK terlebih dahulu sebelum mengajukan reimbursement.";
                                }
                            }
                        }
                        ?>
...
$(document).ready(function() {
    ...
    $('.jenis-petty').each(function() {
        ...
        $.ajax({
            url: "<?= site_url('pengajuan_pettycash/cek_bpkk_ready') ?>",
            type: "POST",
            data: {
                jenis_saldo: jenis
            },
            ...
            success: function(res) {
                ...
                if (infoIcon) {
                    let content = res.allow ?
                        "Ada data BPKK yang sudah selesai, Anda bisa ajukan." :
                        "Upload data dan submit BPKK terlebih dahulu.";
                ...
                // jika belum ada data BPKK selesai → disable pilihan
                if (!res.allow) {
